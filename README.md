...
6. [親機を起動する（全7ステップ）](#6-親機を起動する全7ステップ)
...
### Step 6-6：利用者画面をブラウザで開く（ログイン不要）

利用者は、管理者ログインなしで次の画面を使います。

~~~text
http://localhost:5000/
~~~

別の PC からは次の URL を開きます。

~~~text
https://<親機のホスト名>/
~~~

メイン画面の「学生証を登録する」は <code>/register</code>、「利用状況を確認する」は
<code>/usage</code> に進みます。どちらも管理者アカウントは不要です。

画面の流れと、管理者画面との境界は [docs/user_flow.md](docs/user_flow.md) を参照してください。

### Step 6-7：管理画面をブラウザで開く
...
2. 補充・在庫修正・廃棄は、管理画面の「運用管理」で理由を添えて記録
3. 故障や詰まりは「運用管理」でチケット化し、復旧確認まで状態を更新
4. 子機の設定を変えたら heartbeat の同期完了を確認
5. **管理画面のパスワードはチャットやメモに平文で貼らない**
