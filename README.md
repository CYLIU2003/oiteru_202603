...
**ステップ1: usbipd-winのインストール（Windowsホスト側）**
...
**ステップ2: WSL2にusbipツールをインストール（WSL2内）**

通常のターミナルで`wsl`と入力してWSL2に入り、以下を実行：
...
# 1-4    054c:06c1  RC-S380/S             Not shared
# ↑ このBUSID（例: 1-4）をメモする
```

**ステップ2: NFCリーダーをバインド（初回のみ）**

```powershell
# NFCリーダーのBUSIDをバインド（例: 1-4）
usbipd bind --busid 1-4
```

**ステップ3: NFCリーダーをWSL2にアタッチ（毎回必要）**
...
# 出力例:
# Bus 001 Device 002: ID 054c:06c1 Sony Corp.
# ↑ NFCリーダーが表示されればOK
```
...
# NFCリーダーが正常に動作するか確認
docker exec oiteru_flask python3 -c "import nfc; clf = nfc.ContactlessFrontend(); print('✅ NFC reader OK'); clf.close()"
...
**Q: `usbipd: command not found`と表示される**
- A: PowerShellを**管理者権限**で開いているか確認してください。

**Q: `There is no device with busid 'X-X'`と表示される**
- A: `usbipd list`でBUSIDを再確認してください。デバイスが物理的に接続されているか確認してください。

**Q: WSL2内で`lsusb`が空または`No such file or directory`**
- A: `usbipd attach --wsl --busid X-X`を再実行してください。

**Q: コンテナ内でNFCリーダーが認識されない**
- A: 以下を順番に確認してください：
  1. `wsl lsusb`でWSL2内にデバイスが見えるか
  2. `docker-compose down`してからコンテナを再起動
  3. USBケーブルを抜き差しして`usbipd attach`を再実行
...
# WSL2からUSBデバイスをデタッチ（管理者権限PowerShell）
