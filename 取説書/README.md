# 📚 OITELU ドキュメント# 🍬 OITELU (オイテル) システム



このフォルダには、OITELUシステムのドキュメントが含まれています。**〜 NFCカードで「お菓子」を管理するスマートIoTシステム 〜**
...
| [QUICKSTART.md](QUICKSTART.md) | 🔰 初心者 | まずはここから！ステップバイステップで始められます |

| [REFERENCE.md](REFERENCE.md) | 📖 上級者 | 全機能の詳細説明、API仕様、トラブルシューティング |- **簡単操作**: ICカードをかざすだけのシンプル操作

| [CHANGELOG.md](CHANGELOG.md) | 📝 全員 | 更新履歴 |- **自動登録モード** ⭐ **NEW**: 未登録カードも自動で登録して利用可能！
...
## 🔰 初めての方へ- **複数登録機対応**: 親機と従親機が同じMySQLデータベースを共有
...
以下の内容が含まれています：## � シンプルなファイル構成

- 親機（サーバー）のセットアップ

- 子機（Raspberry Pi）のセットアップチームメンバーが混乱しないよう、**主要ファイルは3つ**に整理しました。
...
## 📖 詳しく知りたい方へ├── 📡 unit.py          ← 子機（NFC読み取り + モーター制御）

│

**[REFERENCE.md](REFERENCE.md)** を読んでください。├── 📄 db_adapter.py    ← データベース抽象化（SQLite/MySQL両対応）

├── 📄 unit_client.py   ← 子機の詳細実装（unit.pyから呼び出し）

以下の内容が含まれています：├── 📄 config.json      ← 子機の設定ファイル

- システム構成の詳細├── 📄 .env.example     ← サーバーの設定ファイル（テンプレート）

- 複数親機構成の設定方法│

- データベース管理├── 🐳 docker-compose.mysql.yml      ← 親機DB版用

- API仕様├── 🐳 docker-compose.external-db.yml ← 従親機用

- 設定ファイルの詳細│

- ハードウェア制御├── 📁 templates/       ← Web画面テンプレート

- Docker設定├── 📁 static/          ← CSS/画像

- トラブルシューティング├── 📁 scripts/         ← 便利スクリプト
...
1. まず **[QUICKSTART.md](QUICKSTART.md)** の「よくある質問」を確認| あなたの役割 | 使用するファイル | 説明 |

2. 解決しない場合は **[REFERENCE.md](REFERENCE.md)** の「トラブルシューティング」を確認|:---|:---|:---|

3. それでも解決しない場合は担当者にお問い合わせください| **親機（DB持ち）を起動したい** | `db_server.py` | MySQLデータベースを含むメインサーバー |
