...
#### 4-1. 設定ファイルの編集
`unit_client.py` を開き、`SERVER_URL` を書き換えます。

```python
# 📝 設定変更箇所
# ステップ2でメモした親機のTailscale IPアドレスを入力
SERVER_URL = "http://100.100.10.10:5000"
```

#### 4-2. 必要なライブラリのインストール
```bash
pip install requests nfcpy
```
