...
def prune_login_attempts():
    ...
    expired = [
        ip
        for ip, attempts in ADMIN_LOGIN_ATTEMPTS.items()
...
def get_authenticated_unit(
    conn, unit_name: str, unit_password: str = None, unit_token: str = None
):
    unit = db.fetchone(conn, "SELECT * FROM units WHERE name = ?", (unit_name,))
    if not unit:
        return None
...
def get_period_display_name(period: str) -> str:
    """期間の表示名を取得"""
    period_names = {"day": "1日", "week": "1週間", "month": "1ヶ月"}
...
def check_and_reset_user_stock(conn, user, period: str) -> dict:
    ...
    if last_reset < period_start:
        ...
        print(
            f"[自動リセット] カードID {card_id} の残数を {reset_stock} にリセット（{period_name}変更）"
        )
        ...
        with get_connection() as conn:
            ...
            if db.db_type == "mysql":
                ...
                try:
                    db.execute(
                        conn,
                        "ALTER TABLE settings ADD COLUMN usage_limit INT DEFAULT 2",
                    )
                ...
                try:
                    db.execute(
                        conn,
                        "ALTER TABLE settings ADD COLUMN limit_period VARCHAR(10) DEFAULT 'day'",
                    )
                ...
                try:
                    db.execute(
                        conn, "ALTER TABLE users ADD COLUMN last_reset_date DATE"
                    )
                ...
                try:
                    db.execute(
                        conn,
                        "UPDATE settings SET usage_limit = daily_limit WHERE usage_limit IS NULL OR usage_limit = 0",
                    )
                except Exception:
                    pass
            else:
                ...
                try:
                    db.execute(
                        conn,
                        "ALTER TABLE settings ADD COLUMN usage_limit INTEGER DEFAULT 2",
                    )
                    print("  - settingsテーブルに usage_limit カラムを追加しました")
                except Exception:
                    pass
                ...
                try:
                    db.execute(
                        conn,
                        "ALTER TABLE settings ADD COLUMN limit_period TEXT DEFAULT 'day'",
                    )
                    print("  - settingsテーブルに limit_period カラムを追加しました")
                except Exception:
                    pass
                ...
                try:
                    db.execute(
                        conn, "ALTER TABLE users ADD COLUMN last_reset_date TEXT"
                    )
                    print("  - usersテーブルに last_reset_date カラムを追加しました")
                    # 既存ユーザーには今日の日付を設定
                    today = datetime.now().strftime("%Y-%m-%d")
        ...
        if card_id:
            try:
                with get_connection() as conn:
                    ...
                    db.execute(
                        conn,
                        "INSERT INTO users (card_id, entry, stock) VALUES (?, ?, ?)",
            ...
            flash(
                "カードを読み取れませんでした。カードをリーダーに置いてください。",
                "warning",
            )
        return redirect(url_for("register"))
...
def admin_history():
    ...
    with get_connection() as conn:
        history = db.fetchall(
            conn,
            "SELECT * FROM history WHERE type = 'success' ORDER BY created_at DESC LIMIT 100",
        )
...
def admin_visuals():
    ...
    with get_connection() as conn:
        ...
        logs = db.fetchall(
            conn,
            "SELECT txt, created_at FROM history WHERE type = ? AND created_at >= ? ORDER BY created_at DESC",
            ("success", valid_start_date.strftime("%Y-%m-%d %H:%M:%S")),
        )
    ...
    chart_data = {
        ...
        "hourly_by_date": {
            date: [hourly_by_date[date][h] for h in range(24)]
            for date in available_dates
        },
        "available_dates": available_dates,
        "daily_labels": sorted(daily_counts.keys())[-14:]
        if daily_counts
        else [],  # 直近14日
        "daily_data": [
            daily_counts.get(d, 0)
            for d in (sorted(daily_counts.keys())[-14:] if daily_counts else [])
        ],
    ...
    return render_template(
        "admin_visuals.html", chart_data=chart_data, total_count=len(timestamps)
    )
...
def api_reader_status():
    ...
    try:
        ...
        return jsonify(
            {"connected": False, "error": f"ステータス取得失敗: {str(e)}"}
        ), 500
...
def api_read_card():
    ...
    try:
        ...
        # 10秒間待機してカードを読み取り
        clf.connect(
            rdwr={"on-connect": on_connect}, terminate=lambda: card_id is not None
        )
        clf.close()
        ...
        if card_id:
            ...
            return jsonify(
                {"success": False, "error": "カードが読み取れませんでした"}
            ), 408
...
def api_unregistered_units():
    ...
    return jsonify({"success": len(units) > 0, "count": len(units), "units": units})
...
def api_toggle_unit_available(uid):
    ...
    try:
        ...
        add_history(
            f"子機(ID:{uid})の利用可能状態を{'有効' if new_available else '無効'}に変更",
            "system",
        )
...
def api_toggle_user_allow(uid):
    ...
    try:
        with get_connection() as conn:
            ...
            db.execute(
                conn, "UPDATE users SET allow = ? WHERE id = ?", (new_allow, uid)
            )

        add_history(
            f"利用者(ID:{uid})の許可状態を{'許可' if new_allow else '不許可'}に変更",
            "system",
        )
...
def api_record_usage():
    ...
    try:
        with get_connection() as conn:
            ...
            unit = get_authenticated_unit(conn, unit_name, unit_password, unit_token)
            if not unit:
                update_dispense_event(
                    conn, event_id, "failed", "INVALID_UNIT_CREDENTIALS"
                )
                return jsonify(
                    {"error": "Invalid unit credentials", "event_id": event_id}
                ), 401
            ...
            if unit["stock"] <= 0 or unit["available"] == 0:
                ...
                return jsonify(
                    {"error": "Unit has no stock remaining", "event_id": event_id}
                ), 400
            ...
            if not user:
                # デバッグログ
                print(
                    f"[DEBUG] 未登録カード: {card_id}, auto_register_mode={server_settings['auto_register_mode']}"
                )
            ...
            if user.get("allow", 1) == 0:
                ...
                return jsonify(
                    {"error": "User is not allowed", "event_id": event_id}
                ), 403
            ...
            if user["stock"] <= 0:
                ...
                return jsonify(
                    {"error": "User has no stock remaining", "event_id": event_id}
                ), 400
    ...
    try:
        with get_connection() as conn:
            ...
            if transitioned == 0:
                latest = db.fetchone(
                    conn,
                    "SELECT status, error_code FROM dispense_events WHERE event_id = ?",
                    (event_id,),
                )
            ...
            latest_unit = db.fetchone(
                conn, "SELECT * FROM units WHERE name = ?", (unit_name,)
            )
            user = db.fetchone(
                conn, "SELECT * FROM users WHERE card_id = ?", (event["card_id"],)
            )
            ...
            db.execute(
                ...
                (new_user_stock, new_total, event["card_id"]),
            )

            new_unit_stock = latest_unit["stock"] - 1
            db.execute(
                conn,
                "UPDATE units SET stock = ? WHERE name = ?",
                (new_unit_stock, unit_name),
            )

            if new_unit_stock <= 0:
                db.execute(
                    conn, "UPDATE units SET available = 0 WHERE name = ?", (unit_name,)
                )
    ...
    with get_connection() as conn:
        ...
        if unit:
            ...
            response_data = {
                ...
                "unit_api_token": unit_session_tokens.get(unit_name)
                or issue_unit_session_token(unit_name),
            }
...
def api_get_unit_config(unit_name):
    ...
    if unit_name in unit_configs:
        return jsonify(
            {"success": True, "unit_name": unit_name, **unit_configs[unit_name]}
        )
    else:
        return jsonify(
            {
...
def api_update_unit_config(unit_name):
    ...
    if unit_name in unit_configs:
        ...
        unit_configs[unit_name]["last_updated"] = datetime.now().strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    else:
        unit_configs[unit_name] = {
    ...
    if unit_ip and unit["connect"] == 1 and push_headers:
        try:
            ...
            response = requests.post(
                unit_url, json={"config": new_config}, headers=push_headers, timeout=5
            )
            ...
            if response.status_code == 200:
                ...
                push_error = (
                    f"子機が設定を受け付けませんでした (status: {response.status_code})"
                )
    ...
    if not push_success:
        add_history(
            f"子機({unit_name})の設定変更を予約（次回heartbeatで同期）", "system"
        )
...
def api_send_unit_command(unit_name):
    ...
    try:
        ...
        response = requests.post(
            unit_url, json={"command": command}, headers=push_headers, timeout=10
        )
        ...
        if response.status_code == 200:
            ...
            return jsonify(
                {"success": True, "result": result, "message": "コマンドを送信しました"}
            )
        else:
            return jsonify(
                {
