...
.PARAMETER MysqlPassword
    MySQLパスワード。未指定時は .env の MYSQL_PASSWORD を使用
...
# db_server.py を標準入口として使用
$ServerScript = $null
if (Test-Path (Join-Path $ProjectDir "db_server.py")) {
    $ServerScript = "db_server.py"
}
elseif (Test-Path (Join-Path $ProjectDir "server.py")) {
    $ServerScript = "server.py"
    Write-Warning "db_server.py が見つからないため legacy server.py を使用します"
}
elseif (Test-Path (Join-Path $ProjectDir "app.py")) {
    $ServerScript = "app.py"
}
else {
    Write-Error "db_server.py / server.py / app.py が見つかりません"
    exit 1
}
